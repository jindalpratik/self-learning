...
pub fn largest_square_area(bottom_left: Vec<Vec<i32>>, top_right: Vec<Vec<i32>>) -> i64 {
    ...
    for i in 0..num_rectangles {
        for j in (i + 1)..num_rectangles {
            ...
            {
                let mut rect: Vec<i64> = Vec::new();
                rect.push(max(bottom_left[i][0], bottom_left[j][0]).into());
                rect.push(max(bottom_left[i][1], bottom_left[j][1]).into());
                rect.push(min(top_right[i][0], top_right[j][0]).into());
                rect.push(min(top_right[i][1], top_right[j][1]).into());
